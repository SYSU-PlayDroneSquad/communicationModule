...
int main(int argc, char** argv)
{
    ...
    while(ros::ok()){
        ...
        // int recv_byte = zmq_msg_recv(&recv_msg,serverList[0],0);//ZMQ_DONTWAIT
        // if (recv_byte > 0)
        // {
        ...
        //     ROS_INFO("Server receive: %s", delaymsg.cmd().c_str());
        //     publishMsg(delaymsg);
        // }
        ...
        for(int i=0;i<numUav;++i){
            ...
            if(recv_byte > 0)
            {
                ...
                delaymsg.ParseFromString(str);
                ROS_INFO("Server recieve: msg_id:%d send time:%.9lf",delaymsg.msg_id(),ros::WallTime::now().toSec()-delaymsg.send_time());
                // 与地面站通信
                if (i == 0)
                {
                    ROS_INFO("Server receive: %s", delaymsg.cmd().c_str());
